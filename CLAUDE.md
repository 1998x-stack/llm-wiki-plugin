...
- **Topic Map** (`.claude/topic-to-wiki.json`): **authoritative** topic → pages mapping, generated by Haiku subagent during `wiki:reindex`. Primary source for `wiki:query` topic expansion and `maps/*.md` generation. Do not edit manually.
- **Maps** (`vault/wiki/maps/`): per-topic knowledge map files generated from `topic-to-wiki.json`. Used by `wiki:query` to broaden search beyond BM25 matches. Topics are semantically derived by LLM, not tag-frequency heuristics.
