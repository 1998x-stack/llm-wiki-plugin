...
上下文压缩是 Anthropic 官方推出的服务端上下文管理策略，当长时对话接近[[上下文窗口]]限制时，自动将旧内容压缩为简洁摘要，替换原始历史记录，从而扩展有效上下文长度。这是 [[Context-Engineering]] 中三种长时任务技术（[[上下文压缩（Context Compaction）|Compact]]ion / [[结构化笔记法]] / 多 Agent 架构）的官方 API 实现。
