...
1. **解决 DIN 的时序缺陷** — DIN 将用户历史行为视为无序集合，忽略行为之间的时序关系和演化规律。DIEN 通过引入 GRU（[[GRU|Gated Recurrent Unit]]）结构显式捕获兴趣随时间的动态变化。
2. **兴趣抽取层（Interest Extractor Layer）** — 使用 GRU 对用户行为序列进行逐时刻编码，每个时间步输出一个兴趣状态向量，形成兴趣演化轨迹。
3. **兴趣演化层（Interest Evolving Layer）** — 引入 [[Target Attention]] 机制，让候选广告与 GRU 的每个隐藏状态进行[[注意力机制|注意力]][[计算]]，动态选择与当前候选最相关的兴趣演化路径。
4. **辅助损失（Auxiliary Loss）** — 为缓解 GRU 训练中的[[梯度消失]]问题，DIEN 设计了辅助点击预测任务：在每个时间步预测用户下一个行为是否为正样本，提供额外的监督信号。
5. **与 DIN 的关系** — DIEN 保留了 DIN 的 [[Target Attention]] 机制，但在[[注意力机制|注意力]][[计算]]之前先用 GRU 对行为序列进行时序编码，实现了"时序建模 + [[注意力机制|注意力]]激活"的两阶段兴趣表示。
