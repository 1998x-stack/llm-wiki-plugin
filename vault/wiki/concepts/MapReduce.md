...
MapReduce 是一种用于大规模数据处理的编程模型，用户只需定义 Map 和 Reduce 两个函数，系统便能自动将[[计算]]并行化并分布到数千台机器上执行。
...
2. **Map 阶段**：每个 [[Worker Agent|Worker]] 处理一个分片，输出中间键值对
...
- **透明容错**：[[Worker Agent|Worker]] 故障自动重新调度
- **数据本地性**：移动[[计算]]而非移动数据
