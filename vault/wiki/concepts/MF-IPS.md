...
## 概述
基于 [[逆倾向评分|IPS]] 的[[矩阵分解]]（[[矩阵分解|MF]]-[[逆倾向评分|IPS]]）将逆倾向加权嵌入经验风险最小化框架，在 [[MNAR]] 数据上实现近似无偏的[[矩阵分解]]训练。
...
4. **与传统的关**：当所有 [[倾向性评分]] 相等时（即 [[MCAR]]），[[矩阵分解|MF]]-[[逆倾向评分|IPS]] 退化为传统的[[矩阵分解]]。这说明传统方法是 [[矩阵分解|MF]]-[[逆倾向评分|IPS]] 在特殊假设下的特例。

5. **实验结果**：在 [[Yahoo! R3]] 和 Coat Shopping 数据集上，[[矩阵分解|MF]]-[[逆倾向评分|IPS]] 显著优于所有基线方法（[[矩阵分解|MF]]-Naive、HL-MAR、HL-[[MNAR]]），配对 t 检验 p < 0.001。[[矩阵分解|MF]]-[[逆倾向评分|IPS]] 的性能甚至超过了计算复杂度远高于它的联合似然方法（HL-[[MNAR]]）。

6. **关键启示**：一个概念上简单、计算上高效的方法（[[逆倾向评分|IPS]] 加权的[[矩阵分解]]），通过正确地处理偏差，可以胜过复杂的联合概率模型。

7. **局限性**：[[矩阵分解|MF]]-[[逆倾向评分|IPS]] 主要关注 [[显式反馈]]（评分）场景。现代推荐系统越来越多地依赖 [[隐式反馈]]（点击、浏览、购买），[[隐式反馈]]中的偏差模式更加复杂。
