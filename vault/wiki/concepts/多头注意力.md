...
多头[[注意力机制|注意力]]（[[Multi-Head Attention]]）在 h 个独立子空间中并行执行 [[Self-Attention机制]]，再拼接并线性变换，使模型同时捕获不同类型的语义关系，是 [[Transformer架构]] 的标准[[注意力机制|注意力]]形式。
...
每个 head 独立执行[[缩放点积注意力]]：

$$head_i = \text{Attention}(QW_Q^i, KW_K^i, VW_V^i)$$

每个 head 的投影维度通常为 $d_k = d_{model}/h$，保持总[[计算]]量与单头相当。

### 为什么需要多头

单头[[注意力机制|注意力]]在单一表示子空间内学习关系。多头机制让模型在不同子空间里同时学习：
...
多头[[注意力机制|注意力]]的总复杂度与单头相同（子空间维度缩小 h 倍，但有 h 个 head）：

- 时间复杂度：$O(n^2 d)$
- 空间复杂度：$O(n^2)$（每个 head 的[[注意力机制|注意力]][[矩阵]]为 $n \times n$）
