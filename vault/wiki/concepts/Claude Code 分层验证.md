...
**只靠 [[CLAUDE.md]] 的三个问题：**
1. [[Claude_Code|Claude]] 可能在复杂任务中"知道但漏做"
2. 做法不一致（有时跑全量、有时跑单文件）
3. 失败时拿不到结构化、自动回灌的反馈

**只靠 tool description 的问题：**
- 决定的是"会不会正确调用"，不是"是不是必定会发生"
- 在 agentic loop 里，[[Claude_Code|Claude]] 会根据任务自己选择工具
...
**特点：** [[CLAUDE.md]] 是"每个会话都会加载的指令层"，适合放长期稳定的项目规则，让 [[Claude_Code|Claude]] 有稳定偏好。但它本身不会自动在写文件后触发命令。
...
- 可以把 `additionalContext` 或 `decision: "block"` 返回给 [[Claude_Code|Claude]]
...
**特点：** 官方 [[Tool System|Tools]] reference 明确写了：LSP 工具提供 code intelligence；**after each file edit, it automatically reports type errors and warnings**。它天然适合作为第一道快速反馈，和 edit 紧耦合，不用额外教 [[Claude_Code|Claude]]"写完后先去哪里看诊断"。
...
很多人把所有规则都写进 [[CLAUDE.md]]，期待 [[Claude_Code|Claude]] 记住"写完就 lint → lint 失败就修 → 再跑 test → 再检查 changed files"。这在简单项目里可行，但一旦任务变长，就容易退化成"知道规则，但执行不稳定"。
...
2. **再配 PostToolUse 钩子**：匹配 Edit 或 [[Write]]，自动跑最轻量的 lint/check，并把结果回灌给 [[Claude_Code|Claude]]
