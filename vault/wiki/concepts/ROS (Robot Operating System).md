...
### 历史演变与局限
ROS 1 起源于斯坦福大学 STAIR 项目中的 Switchyard 框架，后由 [[Willow Garage]] 公司工程化并推广。尽管 ROS 1 极大地促进了学术界和初创企业的创新，但其存在显著局限：**缺乏实时性保证**（基于非实时 TCP/IP）、**安全性缺失**（无认证加密）、**单点故障风险**（ROS Master 崩溃影响重配置）以及**平台兼容性差**（主要依赖 Ubuntu）。
