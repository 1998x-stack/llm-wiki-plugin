...
## 概述
GARCH（广义[[AR 模型（自回归模型）|自回归]][[Conditional Heteroskedasticity|条件异方差]]）模型由 [[Tim Bollerslev]] 于 1986 年提出，在 ARCH 基础上引入条件方差的[[AR 模型（自回归模型）|自回归]]项，仅用三个参数即可捕捉波动率的长期持续性，是金融计量经济学引用量最高的论文之一。
