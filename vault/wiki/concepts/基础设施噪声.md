...
source_count: 1
tags: [evaluation, infrastructure, benchmark, reliability, Lua编程]
...
## 概述
Agentic 编码评测中，基础设施[[Configuration|配置]]差异导致评测结果产生系统性偏差的现象，不同资源[[Configuration|配置]]之间的成功率差距可达 6 个百分点，超过主流模型排行榜上顶级模型之间的差距。

## 关键内容
1. **核心机制**：在 [[SWE-bench]] 和 [[Terminal-Bench 2.0]] 等 Agentic 评测中，运行环境不再是被动容器，而是问题解决过程的核心组件。资源[[Configuration|配置]]不同的 Agent 实际上在"做不同的测试"。
2. **两阶段行为**：
   - **阶段一（1x → 3x）**：额外资源主要解决基础设施可靠性问题（容器被意外杀死），成功率在噪声范围内波动（p=0.40）
   - **阶段二（3x → Uncapped）**：额外资源帮助 Agent 采用需要大量资源的解题策略，成功率跳跃 +4 个百分点
3. **典型策略差异**：富资源策略可安装 pandas + networkx + scikit-learn 等大型依赖库，贫资源策略被迫使用标准库从零实现——不同资源[[Configuration|配置]]在评测不同的模型能力维度。
4. **量化影响**：
