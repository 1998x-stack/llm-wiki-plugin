...
2. **功能直觉**：输出门决定当前时刻的隐藏状态 hₜ 应该包含 [[细胞状态（Cell State）]] 中的哪些信息。在[[Language-Model|语言模型]]中，当需要预测下一个词时，输出门会输出与当前上下文相关的隐藏状态。oₜ 接近 1 时充分暴露[[细胞状态（Cell State）|细胞状态]]信息，接近 0 时隐藏信息。
