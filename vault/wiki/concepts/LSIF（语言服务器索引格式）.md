...
## 概述
Language Server Index Format 是 LSP 的离线版本，将语言服务器的分析结果序列化为图结构，用于代码搜索和静态代码导航，被 GitHub 和 Sourcegraph 广泛采用。
...
2. **与 LSP 的关系**：LSP 是实时交互协议（编辑器↔服务器），LSIF 是静态索引格式（预生成、可查询）。LSIF 相当于 LSP 的"快照"。
