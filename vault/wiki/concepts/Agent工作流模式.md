...
将[[任务分解]]为顺序步骤，每次 LLM 调用处理前一次的输出，可加入程序化检查门（Gate）。
...
**例**：客服查询分流（退款 / 技术支持 / 一般问题）；简单问题路由到 [[Claude 3 Haiku|Haiku]] 4.5，复杂问题路由到 [[Claude-Sonnet-4|Sonnet 4]].5。
...
- 每步从环境获取"ground truth"（工具调用结果、[[代码执行]]结果）
...
- [[防错设计|Poka-yoke]]（[[防错设计|防呆设计]]）：让错误难以发生（如强制使用绝对路径）
