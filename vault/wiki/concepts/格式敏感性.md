...
source_count: 1
tags: [ai-engineering, evaluation, model-behavior, prompt-engineering]
...
3. **在 BrowseComp 分析中的发现**：Anthropic 的实验结果显示 Claude Opus 4.6 存在一定程度的格式敏感性，但没有发现明确的"答案记忆"或有意欺骗的证据。更准确的描述是：BrowseComp 的特定任务格式激活了模型某些在日常使用中未充分表达的搜索策略。
...
5. **与提示工程的关系**：格式敏感性解释了为什么[[提示工程]]有效——模型确实对不同提示格式有不同反应。但这也意味着评测结果可能受提示格式影响，需要在评测设计中控制这一变量。
