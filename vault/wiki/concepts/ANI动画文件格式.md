...
2. **通道类型**：P（Position 位移）、R（Rotation 旋转）、S（Scale 缩放）。纯旋转动画最常见，位移通道通常只有根骨骼才有；查询时 [[channel|Channel]]s 字段统计各类型轨道数量。
3. **使用方式**：通过 [[UrhoXCLI]] 的 `anim-info` 子命令查询文件信息；从 FBX 导入后需验证轨道数和关键帧数是否正确；[[动画重定向]]前需对比源动画与目标模型的骨骼名称。
