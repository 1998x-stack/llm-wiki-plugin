...
source_count: 1
tags: [agent-engineering, mcp, tool-use, code-execution, AI工程]
...
1. **从"描述"到"实践"的跃迁**：
   ...
   - 区别不仅是便利——代码执行允许 Agent 处理大规模数据，用[[Transcript vs Outcome|结果验证]]推理而非依赖语言描述
...
5. **最佳实践**：
   ...
   - 始终在[[安全沙箱]]中执行，不允许直接文件系统写入（除[[Claude Code 沙箱机制|沙箱]]目录外）
   - 对超时和内存[[Settings|设置]]合理限制，审查涉及外部系统的代码
   - 代码执行与文件读写工具组合极为强大：读取文件 → 代码处理 → 写入结果

6. **与 [[MCP]] 的集成**：
   - 通过 [[MCP]] 提供安全的代码执行[[服务]]器
