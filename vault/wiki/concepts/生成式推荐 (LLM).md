...
## 概述
将推荐任务[[重构]]为文本生成问题的 LLM [[规范化理论|范式]]，通过[[Language-Model|语言模型]][[AR 模型（自回归模型）|自回归]]生成推荐结果，由 [[P5 论文]]首次系统性提出。

## 关键内容

1. **[[规范化理论|范式]]定义**：将推荐系统的所有任务——评分预测、[[序列推荐]]、解释生成、评论摘要、直接推荐——统一为"输入一段文本，输出一段文本"的语言生成问题，不再需要专门的[[协同过滤]]层、评分预测头或排序损失函数。

2. **开创性工作**：[[P5 论文]]（[[RecSys 2022]]）首次系统性地论证了"将推荐系统完全[[重构]]为语言处理任务"的可行性，使用 T5 作为骨干模型，将五大推荐任务完整统一到一个[[Language-Model|语言模型]]框架中。
...
5. **技术挑战**：推理效率（[[AR 模型（自回归模型）|自回归]]解码延迟远高于向量内积）；物品词汇表可扩展性（百万/亿级 ID 空间）；ID 语义鸿沟（数字 ID 与[[Language-Model|语言模型]]语义空间脱节）。

6. **后续演进**：从 P5（2022，T5-small/base 60M-223M）到现代方案（2024-2025，[[LLaMA]]/Mistral/Qwen 7B-70B），从 Prompt 模板到[[指令调优]]，从数字 ID 到[[语义 ID]]，从全参数微调到 LoRA/QLoRA 参数高效微调。

7. **学术共识**：P5 帮助社区形成了"语言是连接不同推荐任务的天然桥梁"的重要共识，使"LLM for Recommendation"从小众方向迅速成长为推荐系统领域最热门的研究主题。
