...
## 概述
[[逆向运动学|IK]]Effector 是 [[UrhoX引擎|UrhoX]] 的 [[逆向运动学|IK]] 末端执行器组件，定义 [[逆向运动学|IK]] 链的目标位置与旋转，挂载于需要 [[逆向运动学|IK]] 驱动的末端关节，继承自 Component。

## 关键内容

### 核心概念
[[逆向运动学|IK]]Effector 告诉 [[IKSolver]] "这个关节要达到哪里"。链长（chainLength）决定从此节点向上回溯多少个父节点参与 [[逆向运动学|IK]] 求解，从而控制哪些关节被 [[逆向运动学|IK]] 驱动。
...
| 方法 | 说明 |
|------|------|
| `GetChainLength() / SetChainLength(unsigned)` | [[逆向运动学|IK]] 影响的父节点层数 |
...
| `chainLength` | unsigned | [[逆向运动学|IK]] 链长度（父节点层数） |
...
### 典型用法
- 手部 [[逆向运动学|IK]]：chainLength=2（手→前臂→上臂），跟随武器/交互点
- 脚部 [[逆向运动学|IK]]：chainLength=2（脚→小腿→大腿），落在地形表面
- weight < 1 可实现 [[逆向运动学|IK]] 与动画的混合叠加
