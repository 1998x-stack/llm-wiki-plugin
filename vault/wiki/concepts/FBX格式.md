...
2. **内嵌资产**：FBX 内部包含材质名称（如 `Lambert`、`StandardSurface`、`tripo_material_xxx`）、纹理路径引用、骨骼层级及动画轨道。
3. **导入产物**：导入 UrhoX 引擎时可生成 MDL 模型、XML 材质、纹理、预制体（Prefab）和 ANI 动画文件；各产物按需生成，不传路径参数则跳过。
