...
Chaitin 常数 Ω = Σ 2^{-|p|}（对所有停机程序求和）是"随机程序停机的概率"，一个不可[[计算]]的超越数，其二进制展开的每一位都是[[算法]]随机的，包含了所有有限规模[[停机问题]]的完整信息。
...
2. **不可[[计算]]**：不存在[[算法]]能[[计算]] Ω 的任意精度的近似值
3. **[[算法]]随机**：Ω 的二进制展开的每一位都是[[算法]]随机的——无法从前面任何有限位预测下一位
...
如果你知道了 Ω 的精确值，你就能解决所有有限规模的[[停机问题]]——只需枚举所有程序，逐步[[计算]] Ω 的近似值，当近似值的前 n 位稳定时，你就知道哪些长度 ≤ n 的程序会停机。
