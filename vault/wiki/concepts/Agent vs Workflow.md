...
source_count: 1
tags: [agent-architecture, design-principle, workflow, AI工程]
...
## 概述
[[Anthropic]] 提出的关键架构[[区分]]：Workflow 通过预定义代码路径协调 LLM 和工具（确定性），Agent 由 LLM 动态指导自身过程和工具使用（灵活性）。
...
5. **五种模式的归属**：[[Prompt Chaining]]、Routing、Parallelization、[[多Agent架构|Orchestrator-Worker]]s、Evaluator-Optimizer 都属于工作流范式，是确定性与灵活性的不同平衡点。
