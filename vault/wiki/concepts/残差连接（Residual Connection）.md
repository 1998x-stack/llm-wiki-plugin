...
## 概述 (50-200字符)
一种网络结构设计，将输入x直接加到层的输出上形成H(x)=F(x)+x，使梯度可通过恒等捷径直接流向浅层，有效解决深层网络的退化问题和[[梯度消失]]。
...
5. **广泛影响**：[[残差连接]]已成为现代深度学习的默认组件，被[[Transformer]]（每个子层都有残差）、GPT、BERT、扩散模型U-Net等架构采用。后续衍生出DenseNet（密集连接）、SENet（通道注意力+残差）等变体。
