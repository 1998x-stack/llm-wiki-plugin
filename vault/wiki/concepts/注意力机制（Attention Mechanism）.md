...
[[注意力机制]]使解码器在生成每个目标词时，能动态加权聚焦源序列的不同位置，彻底解决了 [[编码器-解码器架构（Seq2Seq）]] 的固定长度瓶颈问题，是现代 LLM 的核心之源。
...
2. **[[Softmax]] 归一化**：$\alpha_{it} = \frac{\exp(e_{it})}{\sum_j \exp(e_{jt})}$，得到[[注意力机制|注意力]]权重 $\alpha_{it} \in [0,1]$，$\sum_i \alpha_{it} = 1$
3. **加权上下文向量**：$c_t = \sum_i \alpha_{it} h_i$，每个解码时刻专属的动态上下文
4. **Decoder 生成**：$s_t = f(s_{t-1}, y_{t-1}, c_t)$，融合[[注意力机制|注意力]]上下文生成目标词

### 可解释性突破：对齐矩阵

[[注意力机制|注意力]]权重 $\alpha$ 构成**对齐[[矩阵]]**，使机器翻译首次可视化。英法等语序相近语言呈现对角线模式，语序调换场景（如英语→法语"European Economic Area"）呈现反对角线模式。
