...
- target: '[[克劳德·香农]]'
  type: extends
  confidence: 0.95
  note: Shannon 1948年论文开篇以更明确的方式重申了这一原则
- target: '[[通信系统模型]]'
  type: related_to
  confidence: 0.85
  note: 这一立场是通信系统模型的认识论基础
- target: '[[信息熵]]'
  type: related_to
  confidence: 0.85
  note: 熵度量不确定性，与消息的语义无关
supersedes: null
---
...
- 这一原则影响了 20 世纪的科学哲学，尤其是[[实证主义|逻辑实证主义]]对"意义"的讨论
- 正因为不关心"意义"，[[信息论]]才能同时应用于电话、电视、互联网、DNA 序列和[[黑洞]]物理
- Shannon (1948) 完全继承了这一原则，并将其转化为完整的数学框架
- 当代大[[Language-Model|语言模型]]的训练目标（最小化[[交叉熵]]）完全不涉及"理解"语义，只涉及预测下一个 token 的概率分布——这正是 Shannon 语义中立立场的直接延续
