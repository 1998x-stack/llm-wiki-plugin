...
## 概述
基于物理的渲染（PBR）材质系统，通过金属度（[[Meta]]llic）和粗糙度（Roughness）参数模拟真实材质光学特性。
...
3. **常见效果参数组合**：光滑金属（[[Meta]]llic 0.9-1.0，Roughness 0.1-0.2）、磨砂金属（[[Meta]]llic 0.9-1.0，Roughness 0.4-0.6）、光滑塑料（[[Meta]]llic 0.0，Roughness 0.3-0.5）、陶瓷（[[Meta]]llic 0.0，Roughness 0.1-0.3）。
4. **自发光（Emissive）**：在 PBRNoTexture 材质上设置 `MatEmissiveColor`，颜色值大于 1.0 可增强 HDR 发光强度；透明材质使用 `PBRNoTextureAlpha.xml` 并通过 Alpha 通道控制透明度。
