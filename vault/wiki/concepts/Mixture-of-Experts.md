...
## 概述
Mixture-of-Experts(MoE)是一种深度学习架构，训练多个小型专家网络处理输入空间不同区域，通过门控网络决定各专家意见的权重。
...
- 门控网络(Gating Network)决定对给定输入应咨询哪些专家及各专家权重
- 数学表达：`y = Σ g(x)_i * f_i(x)`，其中 `g(x)` 通过 softmax 产生，权重和为 1

### 从 MoE 到多任务学习的演进
- **OMoE**(One-gate MoE)：将 MoE 应用于[[多任务学习]]的直接方式，所有任务共享同一门控网络
- **[[MMoE]]**(Multi-gate MoE)：为每任务配备独立门控网络，实现任务特定的专家选择
...
- LLM 中：token 级别的稀疏路由，通常使用 top-k 门控
- 推荐系统中([[MMoE]])：样本级别的软路由，所有专家都有非零权重
- 核心思想一脉相承：用门控/路由机制决定哪些专家处理当前输入

### 未解之题
- **专家特化 vs 专家冗余**：如何确保不同专家学到不同知识而非彼此重复
- **路由优化不稳定性**：门控/路由网络训练是否稳定，是否出现负载不均衡
