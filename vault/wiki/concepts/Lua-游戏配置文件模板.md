...
2. **五类核心模板**：
   ...
   - **skill**：技能配置，含技能类型、cost（mp/rage/cd/sp）、cast（cast_type/cast_time/channel_time/interruptible）、ta[[ripgrep|rg]]et 目标规则（ta[[ripgrep|rg]]et_type/count/range/select_rule/camp_rule）、effects 效果列表（含 formula/chance/duration）、triggers 触发器、hit 命中表现、cooldown 冷却、limit 使用限制、关联 buffs。
   ...
   - **event**：事件配置，含 event_type/priority、trigger（trigger_type/timing/condition[]）、actions[]（action_type/ta[[ripgrep|rg]]et/params）、limit（count/per_round/per_turn/cooldown）、refs 关联资源（units/skills/buffs/[[bat]]tles）、view 表现。
