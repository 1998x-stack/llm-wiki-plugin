...
source_count: 2
tags: [agent-architecture, engineering-patterns, anthropic, AI工程]
...
2. **五种核心模式**：[[Prompt Chaining]]（[[Prompt Chaining|提示链]]）、[[Routing 模式]]（路由）、[[Parallelization 模式]]（并行化）、[[Orchestrator-Workers 模式]]（编排器-工作者）、[[Evaluator-Optimizer 模式]]（评估器-优化器）。这五种模式与软件工程经典设计模式存在深刻对应关系。
3. **模式与软件工程的对应**：[[Prompt Chaining]] ↔ Chain of Responsibility/Pipeline；Routing ↔ Strategy Pattern；Parallelization ↔ [[Parallelization 模式|Fork-Join]]/[[MapReduce]]；[[多Agent架构|Orchestrator-Worker]]s ↔ [[Orchestrator-Workers 模式|Master-Worker]]/Actor Model；Evaluator-Optimizer ↔ Feedback Control Loop。
4. **工作流 vs Agent 的选择**：[[Agent vs Workflow]] 是关键架构[[区分]]。工作流提供确定性和一致性，Agent 提供灵活性和模型驱动决策。除非任务天然开放且步骤无法预判，否则工作流通常优于自主 Agent。
