...
| **L2** | Episodic Memory | 按阶段压缩的"回合摘要"：决策、证据、结论、pending items | 把"过去发生过什么"变成结构化 checkpoints |
| **L3** | Semantic Memory | [[近似最近邻检索|向量检索]] + 关键词检索 + 元数据过滤的长期知识层 | "需要时再拿"，不常驻 |
...
MemGPT (2023) 将 LLM 上下文类比为操作系统的主存，外部存储类比为磁盘。本架构在此基础上增加了：
- 明确的五层语义定义
- memory object 结构化模型
- 与 Prompt 缓存的协同（L0 [[eza|exa]]ct-prefix match）
- 与混合检索的协同（L3 hybrid retrieve）
