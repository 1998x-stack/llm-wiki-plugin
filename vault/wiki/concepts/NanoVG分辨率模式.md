...
3. **模式A（设计分辨率，进阶）**：在模式B基础上加 `nvgScale(vg, scale, scale)`，scale = `math.min(logicalW/designW, logicalH/designH)`（CONTAIN策略）。支持绝对布局（设计坐标系）与响应式（屏幕坐标系）混合，DPR 在计算中被约掉，本质是设计坐标到物理像素的直接映射。
