...
1. **本质区别**：
   - **Bi-encoder**：`score(q,d) = cos(E_q(q), E_d(d))`。文档向量**离线预计算并固定**，与查询无关。查询时只需计算查询向量再做 [[近似最近邻检索|ANN]] 检索。本质是"在共享向量空间找几何最近点"。
...
4. **实践架构（两阶段）**：
   - 第一阶段：Bi-encoder（BGE/E5/text-embedding）+ [[近似最近邻检索|ANN]]（[[Faiss|FAISS]]/Milvus）→ Top-100，延迟 < 20ms
