...
检索[[Reranking|重排序]]（Retrieval [[Reranking]]）是 RAG 流水线中的精排步骤：在初始召回（粗排）获得大量候选 Chunk 后，用专门的**[[Reranking|重排序]]模型**对每个 Chunk 结合查询重新评分，筛选出最相关的 Top-K 个注入模型。
...
- **消耗[[注意力预算]]**（见 [[注意力预算]]），分散模型[[注意力机制|注意力]]
- 引入噪声，降低回答质量
- 增加 token 成本和延迟

[[Reranking|重排序]]解决"多中取精"的问题。
...
| 因素 | 说明 |
|------|------|
| **延迟** | [[Reranking|重排序]]是额外的运行时步骤，即使并行评分也增加延迟 |
| **成本** | 重排粒度越细（候选越多）成本越高 |
| **粗排规模** | 候选 Chunk 越多（如 150 vs 50），精排潜力越大，但[[计算]]开销增加 |
