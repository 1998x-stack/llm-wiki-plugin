...
1. **核心原则：外部验证，不信任自评估**：
   ...
   - ✅ 正确：启动真实浏览器/[[服务]]器，执行真实操作，验证真实结果
   - LLM 极容易在查看自己写的代码时产生确认偏误，必须通过外部运行时验证打破这一循环

2. **模式一：[[浏览器自动化验证]]（Dev-Browser [[Skills|Skill]] / [[Puppeteer MCP]]）**：
   - 最重要的验证手段，适用于所有前端 UI Story
   - 在 [[CLAUDE.md]] 中声明 [[Skills|Skill]]，引导 Agent 使用 dev-browser 或 [[Puppeteer MCP]]
...
4. **模式二：API 测试（curl 脚本链）**：
   ...
   - [[Environment Variables|环境变量]]支持：`BASE_URL` 可[[Configuration|配置]]，适配不同环境

5. **模式三：自动测试套件集成**：
   - 在 [[项目约定手册|AGENTS.md]] 中记录测试命令，Agent 每次迭代后运行
...
7. **验证失败处理树**：
   ...
   - 测试脚本问题（测试本身有 bug）→ 修复测试脚本 → 记录到 [[项目约定手册|AGENTS.md]] → 重新验证
...
8. **验证命令速查表**：
   ...
   - [[服务]]器健康检查：`curl -s http://localhost:3000 -o /dev/null -w "%{http_code}"`（期望 200）
