...
TCP/IP 是全球[[互联网]]的核心协议族，由 [[Vinton Cerf]] 和 [[Robert Kahn]] 于1974年提出，通过端到端可靠传输和无状态网络互联，使得异构网络能够无缝互联。
...
- **IP（[[TCP_IP|Internet Protocol]]）**：网际层，负责寻址和路由。无连接、不可靠、尽力交付
- **TCP（[[TCP_IP|Transmission Control Protocol]]）**：传输层，负责端到端的可靠、有序、面向连接的数据传输
...
截至2024年，全球55亿[[互联网]]用户全部运行在 TCP/IP 之上。从超级[[计算]]机到智能手表，从海底光缆到低轨卫星，TCP/IP 是唯一通用的通信协议。
