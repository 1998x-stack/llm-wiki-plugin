...
## 概述
[[DIN]] 论文提出的数据自适应激活函数，通过引入输入统计量将分界点从固定的零移动到数据分布中心，可视为 [[Probabilistic Robotics|PR]]eLU 的泛化。

## 关键内容

1. **核心公式** — $\text{Dice}(s) = p(s) \cdot s + (1 - p(s)) \cdot \alpha \cdot s$，其中 $p(s) = \frac{1}{1 + e^{-\frac{s - E(s)}{\sqrt{\text{Var}(s) + \epsilon}}}}$ 是 Sigmoid 控制函数。
2. **数据自适应分界点** — 传统 ReLU/[[Probabilistic Robotics|PR]]eLU 的分界点固定在零，Dice 通过引入输入均值 $E(s)$ 和方差 $\text{Var}(s)$，使分界点自适应到数据分布中心，适应每一层不同的输入分布。
