...
4. **BN 的隐式作用** — 公式中的标准化项 $(s - E(s)) / \sqrt{\text{Var}(s) + \epsilon}$ 本质上就是 [[Batch Normalization]] 操作，使 Dice 天然具有缓解 [[内部协变量偏移|Internal Covariate Shift]] 的能力。
5. **与 PReLU 的退化关系** — 当 $E(s) = 0$ 且 $\text{Var}(s) = 0$ 时，Dice 退化为 PReLU，因此 Dice 是 PReLU 的泛化形式。
6. **训练与推理** — 训练阶段使用当前 mini-batch 的统计量[[计算]] $E(s)$ 和 $\text{Var}(s)$；推理阶段使用训练过程中的指数移动平均值，与 BN 的处理方式一致。
