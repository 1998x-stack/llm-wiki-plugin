...
Jacobi 迭代法是求解线性方程组 $Ax=b$ 的经典迭代方法，由[[卡尔·古斯塔夫·雅各布·雅可比]]约于 1845 年提出。核心思想是将系数矩阵分裂为对角部分与非对角部分（$A=D+L+U$），利用旧值同步更新所有分量，在对角占优等条件下保证收敛。其"同时更新"特性使它天然适合并行计算，并开创了迭代求解线性方程组的整个范式。
...
**充要条件**：Jacobi 迭代收敛当且仅当迭代矩阵 $B_J = -D^{-1}(L+U)$ 的[[谱半径]] $\rho(B_J) < 1$。
