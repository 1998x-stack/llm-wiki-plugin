...
Jacobi 迭代法是求解线性方程组 $Ax=b$ 的经典迭代方法，由[[卡尔·古斯塔夫·雅各布·雅可比]]约于 1845 年提出。核心思想是将系数[[矩阵]]分裂为对角部分与非对角部分（$A=D+L+U$），利用旧值同步更新所有分量，在对角占优等条件下保证收敛。其"同时更新"特性使它天然适合并行[[计算]]，并开创了迭代求解线性方程组的整个[[规范化理论|范式]]。
...
动机来自 19 世纪天文学：行星轨道摄动理论需要求解大规模但对角占优的线性方程组，直接法的 $O(n^3)$ [[计算]]量在手工[[计算]]时代难以承受。相关思想散见于雅可比 1845 年（Astronomische Nachrichten）和 1846 年（Crelle's Journal）的论文——"Jacobi 迭代法"是后人对其方法的总结命名，并非雅可比自己的称谓。
