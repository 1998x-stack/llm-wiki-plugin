...
ACI 设计原则是类似人机界面（HCI）的[[ACI (Agent-Computer Interface)|工具接口设计]]方法论，源自 [[SWE-agent]] 论文的四条核心原则，并在 [[Anthropic]] 工程实践中得到进一步验证和扩展，强调接口设计对 Agent 行为的决定性影响。
...
- 少量、清晰、功能边界明确的动作更好
- 背后的逻辑：[[Language-Model|语言模型]]对"语义上明确的工具"更容易形成稳定使用模式，而对几乎无限制的 shell 容易出现动作选择混乱
...
- [[Guardrails]] 可以减少[[错误复合|错误传播]]、加速恢复
...
1. **ACI 概念起源**：在 Building Effective [[Agents]] 中首次提出，将[[ACI (Agent-Computer Interface)|工具接口设计]]类比为传统的人机界面（HCI），强调接口设计对 Agent 行为的决定性影响。

2. **AI 辅助工具描述改进**：2025 年 9 月提出的 Writing [[Tool System|Tools]] for [[Agents]] 方法，利用 AI 自我改进工具描述，实现任务完成时间降低 40%。这是 ACI 设计的自动化演进路径。

3. **工具设计对 Agent 行为的影响**：[[上下文工程]]研究表明，[[ACI (Agent-Computer Interface)|工具接口设计]]直接影响 Agent 的上下文使用效率和任务完成率。绝对路径案例证明微小设计差异导致显著性能差异。

4. **与 MCP 的关系**：[[MCP（Model Context Protocol）]]是 ACI 设计的具体实现标准，通过 [[Desktop Extensions]] 实现一键安装 [[MCP 服务器]]，降低生态系统摩擦。

5. **[[并行工具调用]]与 tool_choice 控制**：2025 年 11 月提出的高级工具使用模式，支持并行执行和精确控制，是 ACI 设计的高级形态。
