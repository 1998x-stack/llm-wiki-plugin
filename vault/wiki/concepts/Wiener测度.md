...
## 概述
Wiener 测度是[[诺伯特·维纳]]于1923年在连续函数空间 $C_0[0,1]$ 上构造的概率测度，是无穷维空间上第一个严格的概率测度，奠定了[[Wiener过程]]（布朗运动）的数学存在性。
...
3. **与 Kolmogorov 扩展定理的关系**：Wiener 的方法后来被理解为[[概率公理体系|Kolmogorov 扩展定理]]的应用范式——先给出满足相容性条件的有限维族，再取极限。维纳的工作（1923）比柯尔莫哥洛夫建立公理体系（1933）早十年。

4. **无穷维概率论的先驱**：Wiener 测度开创了无穷维空间上概率论的研究，影响了高斯过程理论、抽象 Wiener 空间（Gross, 1965）和 Malliavin 微积分（1976）。
