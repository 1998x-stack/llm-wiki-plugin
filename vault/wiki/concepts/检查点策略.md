...
## 概述
检查点策略是 [[Agent Harness模式|Agent Harness]] 设计中的关键原则，通过可验证的客观状态（而非 Agent 自我报告）来判断任务进度和完成标准，防止过早声明完成并支持从中间点恢复。
...
3. **防止过早声明完成**：
   ...
   - **对策**：
     - 在 Harness 中[[Settings|设置]]独立的完成验证机制
     - 对"任务完成"[[Settings|设置]]强制确认步骤
     - 依赖客观指标而非 Agent 自我报告
