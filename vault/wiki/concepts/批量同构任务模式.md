...
source_count: 1
tags: [AI, Agent系统, 设计模式, 并行计算, AI工程]
...
3. **工作流程**：主 Agent 枚举任务 → 生成 CSV → 调用 spawn_agents_on_csv → 等待所有 worker 完成 → 汇总结果生成报告。每个 [[Worker Agent]] 拥有独立上下文和独立[[Claude Code 沙箱机制|沙箱]]，不共享 token 或文件系统。
