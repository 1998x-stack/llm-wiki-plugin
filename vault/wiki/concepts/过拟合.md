...
4. **预防措施**：使用正则化（L1/L2正则化）、增加训练数据、减少模型复杂度、使用交叉验证、提前停止训练、集成方法（如[[随机森林]]、梯度提升）等。

5. **在[[模型选择]]中的重要性**：AIC等信息准则通过在似然值基础上加入复杂度惩罚项，有效防止过拟合。AIC的第一项-2ln(L)衡量拟合度，第二项2k作为复杂度惩罚，防止模型过度拟合数据。
