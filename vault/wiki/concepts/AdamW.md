...
3. **实践[[Settings|设置]]**：[[Transformer 架构|Transformer]]/NLP 任务推荐 lr=1e-4~3e-4, weight_decay=0.01，配合 Warmup（学习率线性增长到峰值）和 Cosine Annealing（余弦衰减到最小值）。梯度裁剪（clip_grad_norm）防止[[梯度爆炸]]，是 [[Transformer 架构|Transformer]] 训练的必备组件。
