...
source_count: 3
tags: [e2e-testing, frontend, verification, browser-automation, AI工程]
...
4. **在 [[Agent 迭代循环]] 中的位置**：验证步骤位于实现功能之后、更新 prd.json 之前。验证通过 → [[Settings|设置]] passes: true → [[Git Commit|Git 提交]] → 更新 progress.txt。验证失败 → 进入 Bug 处理流程。

5. **截图的双重作用**：
   - **验证证据**：before/after 截图作为验证通过的客观证据
   - **调试辅助**：失败时截图帮助后续 Agent 实例理解问题所在

6. **与 [[Session 交接机制]] 的关系**：验证结果（PASS/FAIL）记录在 progress.txt 中，截图作为 [[Git Commit|Git 提交]]的一部分持久化，供后续 Agent 实例参考。
