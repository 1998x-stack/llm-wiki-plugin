...
Weyl特征值不等式（1912）是[[矩阵扰动理论]]的奠基性定理，由[[赫尔曼·外尔]]在证明偏微分方程特征值渐近律的过程中作为技术工具发展而来。**核心结论：Hermitian [[矩阵]] $A$ 和 $B$ 之和 $A+B$ 的每个特征值，受 $A$ 和 $B$ 各自特征值的精确线性控制；扰动[[矩阵]]将特征值偏移量限制在谱范数量级内。** 该定理将 Hermitian 特征值揭示为[[矩阵]]空间上 Lipschitz 连续的函数，是数值[[线性代数]]所有对称特征值[[算法]]正确性分析的数学基础。
...
| 应用场景 | Weyl不等式的作用 |
|---------|--------------|
| 数值特征值[[算法]]（QR、分治、MRRR） | [[后向误差分析|后向稳定性]]分析：舍入误差 $\leq \|E\|_2$ 界定[[计算]]精度 |
