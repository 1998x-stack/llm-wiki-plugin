...
## 概述
[[MemPalace]] MCP Server 的三个核心工程设计原则：无状态架构、小粒度工具拆分、错误友好性，共同构成高可靠 Agent 工具层。

## 关键内容
- **无状态 Server**：MCP Server 本身不维护任何会话状态，所有状态存储在 [[ChromaDB]] 和 KG 文件中。Server 可随时重启而不丢失数据，符合云原生无状态服务最佳实践
...
- **跨模型兼容**：MCP Server 遵循标准协议，支持 Claude（原生）、GPT-4o、[[Gemini CLI|Gemini]]、Llama（via [[Ollama]]）、Mistral 等多种模型
