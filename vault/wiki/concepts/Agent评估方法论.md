...
[[Anthropic]] 从内部实践和客户协作中提炼的 Agent 系统评估（Eval）系统方法论：词汇体系、[[评分器设计|评分器]]类型、能力评估与回归评估、[[pass@k vs pass^k]] 非确定性指标、以及从零构建评估体系的路线图。
...
| **[[评分器设计|评分器]]（[[评分器设计|Grader]]）** | 评分某方面性能的逻辑；一个任务可含多个[[评分器设计|评分器]] |
...
**代码[[评分器设计|评分器]]（Code-based）**：
- 字符串匹配、正则、二值测试（fail-to-pass）
- 静态分析（lint、类型、安全）、[[Transcript vs Outcome|结果验证]]、工具调用验证
- 优势：快速、客观、可复现、易调试
- 劣势：对有效变体脆弱，缺乏细微判断

**模型[[评分器设计|评分器]]（Model-based / [[LLM-as-Judge]]）**：
- 基于 Rubric 评分、自然语言断言、成对比较、多评判员共识
- 优势：灵活、可扩展、捕捉细微差别
- 劣势：不确定性、成本较高、需用人类[[标注]]校准
- 建议：为每个维度单独调用 LLM judge；给 LLM 提供"Unknown"逃生路线防止幻觉

**人类[[评分器设计|评分器]]（Human）**：
- 领域专家审查、众包判断、A/B 测试
- 优势：黄金标准质量，用于校准模型[[评分器设计|评分器]]
- 劣势：昂贵、慢、难以大规模
...
- 应接近 100% 通过率，下降信号有问题
- **eval 饱和（[[评测饱和|Eval Saturation]]）**：通过率接近 100%，不再提供改进信号；需要更难的评估

### pass@k vs pass^k 指标

应对 Agent 行为非确定性的两个互补指标：

**[[pass@k vs pass^k|pass@k]]**：k 次尝试中至少一次正确的概率
- k 增大 → [[pass@k vs pass^k|pass@k]] 升高
- 适合"一次成功就行"的工具场景（[[代码生成]]）

**[[pass@k vs pass^k|pass^k]]**：k 次尝试全部正确的概率
- k 增大 → [[pass@k vs pass^k|pass^k]] 下降
- 适合"需要每次可靠"的面向用户场景（客服 Agent）

两者在 k=1 时相同；随 k 增大急剧分化——[[pass@k vs pass^k|pass@k]] 趋向 100%，[[pass@k vs pass^k|pass^k]] 趋向 0%。
...
3. **编写无歧义任务与参考解答**：两名领域专家能独立达成相同 pass/fail 判断；创建已知可通过的参考解答验证[[评分器设计|评分器]]
4. **构建平衡问题集**：同时测试"应该触发"和"不应该触发"的情况，避免单侧优化

**Harness 和[[评分器设计]]：**
5. **构建稳健的评估 Harness**：每次试次从干净环境出发；防止跨试次共享状态
6. **[[评分器设计]]**：优先使用确定性[[评分器设计|评分器]]；LLM judge 在必要时使用并用人类校准；支持部分分数；**评估轨迹而非具体路径**（Agent 可能发现有效的非预期解法）

**长期维护：**
7. **阅读 Transcript**：分辨是真正的错误还是[[评分器设计|评分器]]拒绝了有效解法；验证评估确实在测量重要指标
8. **监控饱和并持续贡献**：eval 套件是活的工件，需明确所有权和持续维护

### 不同 Agent 类型的评估策略

**编码 Agent**：确定性[[评分器设计|评分器]]（[[单元测试]] pass/fail）是自然选择；可加 LLM Rubric 评估代码质量和交互方式

**对话 Agent**：多维成功（状态检查 + Transcript 约束 + LLM Rubric）；常需第二个 LLM 模拟用户

**研究 Agent**：接地性检查 + 覆盖度检查 + 来源质量检查；LLM judge 需频繁用专家人类判断校准

**[[计算]]机使用 Agent**：需要在真实或[[Claude Code 沙箱机制|沙箱]]环境运行；URL/页面状态检查 + 后端状态验证（确认行动已实际发生）
...
类似安全工程中的"[[瑞士奶酪模型]]"：没有单一评估层能捕捉所有问题，多[[层组合]]使失误无处遁形。
