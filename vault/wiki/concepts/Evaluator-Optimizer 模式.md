...
source_count: 1
tags: [agent-pattern, workflow, feedback-loop, iteration, Lua编程]
...
## 关键内容
1. **工作流程**：Input → [[[生成器|Generator]]] → [Evaluator] → Pass? → Output（若未通过则反馈给 [[生成器|Generator]] 重新生成）。形成闭环反馈循环。
2. **适用场景的两个判断标准**：
   - LLM 响应在人工反馈后可明显改进
   - LLM 能提供有效的反馈
3. **典型案例**：文学翻译（翻译器 + 评论器）；复杂搜索任务（多轮搜索+分析）；[[代码生成]]与审查循环。
4. **工程要点**：Evaluator 需要有明确的评估标准，避免无限循环。应[[Settings|设置]]最大迭代次数和收敛条件。
