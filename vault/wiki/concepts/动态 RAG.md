...
3. **在研究系统中的应用**：Anthropic 的 Research 功能采用动态 RAG 模式，多 Agent 系统通过子 Agent 并行探索不同方向，每个子 Agent 独立进行动态检索，再将压缩后的信息传递给主 Agent 综合。

4. **信息论优势**：动态 RAG 的本质是信息压缩——从海量语料中提炼洞见，子 Agent 在独立上下文窗口中并行探索，压缩信息后再传递给主 Agent，减少路径依赖，避免陷入局部最优。
