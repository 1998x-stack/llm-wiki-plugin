...
2. **轻量特性**：相比操作系统线程，goroutine非常轻量，可以轻松创建数千甚至数万个而不会耗尽系统资源。

3. **调度机制**：由Go运行时系统调度，使用M:N调度模型（M个goroutine运行在N个操作系统线程上）。

4. **独立性**：每个goroutine拥有自己的栈空间和执行状态，符合CSP中进程拥有私有状态的设计原则。

5. **通信方式**：goroutine之间通过channel进行通信，而不是通过共享内存，实现了CSP"不要通过共享内存来通信，而要通过通信来共享内存"的原则。

6. **工程价值**：使Go语言能够高效地处理大量并发任务，成为现代高并发服务器开发的重要工具。
