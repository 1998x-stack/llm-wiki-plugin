...
1. **三层加载机制**：
   - **第一层：描述**——[[Claude_Code|Claude]] 扫描[[Skills|技能]]目录，根据描述判断 skill 是否匹配当前任务
   - **第二层：[[Agent Skills|SKILL.md]]**——读取核心说明和指令
   - **第三层：支持文件**——如有需要，再加载脚本、模板、参考资料

2. **加载流程**：
   - [[Claude_Code|Claude]] 扫描[[Skills|技能]]目录
   - 根据描述判断是否匹配当前任务
   - 读取 `SKILL.md`
   - 如有需要，再加载脚本和辅助文件

3. **设计动机**：[[上下文窗口]]有限，渐进式披露确保只加载当前任务真正需要的信息，避免[[Context Window Pollution|上下文污染]]和 token 浪费。
