...
source_count: 1
tags: ["示例管理", "manifest", "UrhoX", "项目配置", "文件复制"]
...
## 概述
[[UrhoX引擎|UrhoX]] 中复制示例目录前必须检查 `manifest.json` 文件，确保完整复制所有必需的配置和资源文件。

## 关键内容
1. **检查流程**：复制示例目录前先检查是否存在 `manifest.json`。如果存在，按其 `includes` 字段复制所有匹配文件；如果 `copyAll: true`，复制整个目录（包括非 .lua 文件）；如果不存在，先用 `ls -la` 检查目录结构。
2. **重要文件类型**：`.fsm`（有限状态机）、`.blendspace`（混合空间）、`.json`（配置文件）等非代码文件同样重要，遗漏会导致示例无法正常运行。
