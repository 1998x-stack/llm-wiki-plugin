...
## 概述
赤池信息量准则由日本统计学家[[赤池弘次]]于 1974 年提出，通过平衡模型拟合优度和复杂度来评估统计模型的相对质量，是统计学领域被引用最多的准则之一，彻底改变了[[模型选择]]的方法论。

## 关键内容

1. **历史背景**：1974 年前，[[模型选择]]依赖假设检验（如似然比检验），存在只能比较嵌套模型、阈值任意性、无法同时比较多个模型、忽视预测能力等根本缺陷。[[赤池弘次]]从[[信息论]]角度给出了全新思路。
...
4. **影响**：被广泛应用于[[时间序列分析]]（[[ARIMA 模型|ARIMA]] 阶数选择）、机器学习[[模型选择]]、生态学、经济学等领域。衍生出 AICc（小样本修正）、BIC 等变体。
