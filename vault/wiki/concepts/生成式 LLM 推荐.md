...
## 概述
使用大语言模型进行[[生成式推荐]]的[[规范化理论|范式]]，通过 LLM [[AR 模型（自回归模型）|自回归]]生成推荐结果，是 P5 [[规范化理论|范式]]的 LLM 时代延续。

## 关键内容

1. **[[规范化理论|范式]]定义**：生成式 LLM 推荐（GLLM4Rec）使用大语言模型（LLaMA、Mistral、Qwen 等 7B-70B 参数）作为骨干，通过[[AR 模型（自回归模型）|自回归]]生成推荐结果——物品 ID、推荐列表或推荐理由文本。
