...
source_count: 1
tags: [Lua, 事件系统, tolua++, UrhoX, 数据访问, Lua编程]
...
2. **常用事件字段**：`TimeStep`（[[Delta-time运动模式|delta time]]，用 `GetFloat()`）；`X`/`Y` 等坐标值（用 `GetInt()` 或 `GetFloat()`）。完整事件字段定义见 `.emmylua/Events.d.lua`。
