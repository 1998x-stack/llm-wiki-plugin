...
## 概述
Pearson 相关系数（Pearson Correlation Coefficient）是衡量两个变量之间线性相关程度的统计量，取值范围 [-1, 1]，在推荐系统中被 [[GroupLens]] 系统首次用于计算用户间评分模式的相似度，成为 [[User-Based 协同过滤]]的默认相似度度量。
...
4. **在 [[GroupLens]] 中的应用**：[[GroupLens]] 论文（1994）首次将 Pearson 相关系数引入[[协同过滤]]，用于计算 Usenet 新闻组用户间的相似度。此后十余年间，Pearson 相关系数一直是 [[User-Based 协同过滤]] 的默认相似度度量。

5. **局限性**：
   - 需要至少两篇共同评分的物品才能计算，实际中需要远多于此才能获得稳定估计
   - 只捕捉线性相关，无法检测非线性关系
   - 对异常值敏感
