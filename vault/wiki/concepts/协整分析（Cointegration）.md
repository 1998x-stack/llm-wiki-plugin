...
## 概述
协整分析由 Engle 和 Granger（1987）正式定义，Johansen（1988）提出基于[[最大似然原理|最大似然估计]]的系统性框架，解决了非平稳时间序列之间长期均衡关系的建模问题，是宏观计量经济学的核心工具。
...
3. **Johansen 方法**：基于向量[[AR 模型（自回归模型）|自回归]]（VAR）的[[最大似然原理|最大似然估计]]框架，提供迹检验和最大特征值检验，可同时检测多个协整向量，克服了 Engle-Granger 两步法的局限。
