...
## 概述
XOR（异或）是单层感知机无法解决的经典线性不可分问题，由 Minsky 和 Papert 在 1969 年《[[Perceptrons (Minsky & Papert 1969)|Perceptrons]]》中严格证明，直接导致 [[AI 寒冬]]。
...
5. **解决方案**：XOR 问题最终通过[[多层感知机（MLP）]]解决——添加隐藏层并引入非线性激活函数（如 sigmoid、ReLU），配合[[反向传播（Backpropagation）]]算法训练，即可学习任意非线性决策边界。
