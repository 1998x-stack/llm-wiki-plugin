...
Solomonoff 先验（[[算法]]概率）M(x) = Σ 2^{-|p|} 是对所有能输出 x 的程序按其长度加权求和，是 Occam 剃刀的精确数学化，为通用归纳推理提供了最优先验分布。
...
[[算法]]概率的负对数近似等于 [[安德烈·柯尔莫哥洛夫|Kolmogorov]] 复杂性。一个字符串的先验概率主要由能产生它的最短程序决定。

### Solomonoff 收敛定理

如果数据由某个可[[计算]]的概率分布 μ 生成，Solomonoff 预测器的预测概率会以指数速度收敛到 μ 的真实条件概率，且总预测误差有界：

$$D_{\text{KL}}(\mu \| M | x_1 \cdots x_n) \leq K(\mu) \cdot \ln 2$$

### 不可计算性

M(x) 是不可[[计算]]的——不存在[[算法]]能在有限时间内精确[[计算]]它。这涉及对所有可能程序的求和，等价于解决[[停机问题]]。但它提供了一个理论上限（gold standard），实际学习[[算法]]可视为其可[[计算]]近似。
