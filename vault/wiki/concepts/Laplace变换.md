...
它将时域（或概率域）中的函数 $f(t)$ 转换为复频域的函数 $F(s)$，使微分方程变为代数方程，从而极大地简化分析。Laplace 变换是[[生成函数]]从离散到连续函数的自然推广，也是概率论中矩母函数（moment generating function）的早期形式。
...
- **与概率论的联系**：在概率论中，若 $X \geq 0$ 为随机变量，则 $\mathcal{L}[\text{PDF}_X](s) = \mathbb{E}[e^{-sX}]$ 即为矩母函数，控制矩的生成和分布的唯一性
