...
source_count: 1
tags: [分布式系统, 架构模式, 长事务, Agent工程, Agent系统]
...
## 概述
Saga 模式是[[分布式系统]]中处理长事务的设计模式，将长事务分解为一系列可补偿的短步骤，每步成功后记录进度，失败时从最近检查点回滚或继续，与 [[Agent Harness模式|Agent Harness]] 设计高度相似。
...
2. **与 [[Agent Harness模式|Agent Harness]] 的类比**：
   - [[Agent Harness模式|Agent Harness]] 设计与 Saga 模式高度相似
   - 长时 Agent 任务 ≈ 分布式长事务
