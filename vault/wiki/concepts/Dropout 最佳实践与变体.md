...
## 关键内容 (≥300字符, 用[[双链]])
1. **丢弃率选择最佳实践**：全连接层通常使用 0.5 的丢弃率（参数多易[[过拟合（Overfitting）|过拟合]]），卷积层使用 0.1-0.3 的较低丢弃率（权重共享已有正则效果），LSTM/RNN 中用于非循环连接使用 0.2-0.5，[[Transformer 架构|Transformer]] 中通常使用 0.1，输出层通常不用 [[Dropout]]。小数据集上过高的丢弃率可能导致[[欠拟合]]。
