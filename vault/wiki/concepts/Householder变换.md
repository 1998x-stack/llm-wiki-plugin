...
[[阿尔斯顿·豪斯霍尔德|Householder]] 变换（[[阿尔斯顿·豪斯霍尔德|Householder]] Reflection）由[[阿尔斯顿·豪斯霍尔德]]于 1958 年提出。定义为 $H = I - 2vv^T/v^Tv$——关于与法向量 $v$ 正交的超平面的镜像反射。核心性质：**正交[[矩阵]]**（$H^T H = I$）、**对合**（$H^2 = I$）、**[[条件数]] = 1**（不放大误差）。通过恰当选择 $v$，可以将任意向量 $x$ 一次反射到坐标轴方向（消去多个分量），这使其成为比 Givens 旋转（每次消去一个元素）更高效的 QR 分解工具，计算量从 $O(n^4)$ 降至 $O(n^3)$。
