...
[[Claude Code]] 的[[Context-Engineering|上下文工程]]本质是：**在 [[LLM-Statelessness|LLM 无状态性]]约束下，把有限的 token 窗口从"聊天历史容器"[[重构]]为"可[[计算]]状态机"。** 这需要三件事同时发生：结构化记忆分层（L0–L4）、渐进式检索（三层按需展开）、和生命周期钩子（Hook 系统捕获与压缩）。
...
| L1 [[工作记忆|Working Memory]] | 当前任务、约束、打开的文件 | Context Hook 注入 |
...
|---|---|
| L0 Cached Prefix | 10% |
| L1 [[工作记忆|Working Memory]] + Tool Delta | 35% |
| Recent Turns | 15% |
