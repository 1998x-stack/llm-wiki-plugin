...
**混淆陷阱**：用子串匹配衡量语义等价（漏报），或用 LLM Judge 评估固定格式（成本浪费）。
...
**设计哲学**：先问"规则能否表达"，再决定是否需要 LLM。Judge 是最后手段，不是默认选项。准则粒度要细——一条准则一个可判定问题，失败时 comment 才能定位。

---

## 第三分离：能力 vs 基础设施

**混淆陷阱**：沙箱 OOM / 超时导致的失败被统计为"模型能力退步"，污染横向对比数据。
...
| 确定性 vs 语义 | 过度依赖 LLM（贵）或过度依赖子串（漏） | 需要判断每个准则的表达方式 | 成本与覆盖率的帕累托最优 |
