...
2. **核心能力**：ScaNN（Scalable Nearest Neighbors）是 [[Google]] 专门为大规模推荐系统优化的 [[近似最近邻检索|ANN]] 库。相比通用 [[近似最近邻检索|ANN]] 库，ScaNN 在大规模推荐场景下提供了更好的速度-精度权衡，支持 Anisotropic Vector Quantization 等高级算法。

3. **在推荐系统中的角色**：与 [[Faiss]] 类似，ScaNN 用于在 [[双塔模型]] 产生的物品 embedding 空间中快速找到与用户 embedding 最近的 Top-N 个物品，是 [[候选生成]] 阶段的核心检索组件。

4. **与现代检索技术的关系**：当前的 [[近似最近邻检索|ANN]] 检索技术（如 HNSW、IVF-PQ、ScaNN）已经比2016年时的方案在速度和精度上提升了数个量级。
