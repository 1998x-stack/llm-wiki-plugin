...
LangChain 官方开源的 **生产级 Agent Harness**（`langchain-ai/deepagents`），基于 [[LangGraph]] 构建。定位"batteries-included"——不强迫用户从零拼装，通过可组合的中间件、后端协议和默认系统提示，快速得到"像 Claude Code 一样能干活"的智能体，再按需裁剪扩展。
...
| 子包 | 职责 |
|------|------|
| `libs/deepagents/` | 核心 SDK：`create_deep_agent`、中间件、后端协议 |
| `libs/cli/` | 基于 Textual 的终端 TUI，延迟加载重型依赖 |
| `libs/acp/` | Agent Client Protocol 服务端集成 |
| `libs/evals/` | 评估套件，含 Harbor、Terminal Bench 集成 |
| `libs/repl/` | REPL 中间件（`langchain-repl`） |
