---
type: entity
status: active
confidence: 0.8
created: 2026-04-26
updated: 2026-04-26
last_accessed: 2026-04-26
source_count: 2
tags: [conference, recommendation-systems, acm]
aliases: [RecSys 2016, 第10届ACM推荐系统会议]
relates_to:
  - {target: "[[Deep Neural Networks for YouTube Recommendations]]", type: published_at}
  - {target: "[[Paul Covington]]", type: featured_author}
  - {target: "[[YouTube]]", type: featured_research}
supersedes: null
entity_type: conference
---

# RecSys 2016

## 概述
第10届ACM推荐系统会议（ACM Conference on Recommender Systems），2016年9月举办，录取率仅为18%（159篇投稿录取29篇）。

## 关键内容

1. **会议信息**：
   - 正式名称：Proceedings of the 10th ACM Conference on Recommender Systems
   - 举办时间：2016年9月
   - 录取率：18% (159篇投稿录取29篇)
   - 页码范围：pp. 191-198

2. **历史意义**：
   - 在RecSys历史上，很少有哪篇论文能够像YouTube的《Deep Neural Networks for YouTube Recommendations》一样深刻影响学术研究方向和工业界系统架构
   - 该会议发表了多篇对推荐系统领域产生深远影响的论文

3. **知名论文**：
   - 《Deep Neural Networks for YouTube Recommendations》（Paul Covington等人）：奠定了工业推荐系统十年架构范式的经典论文

## 来源
- [[07-youtube-dnn]] — 深度解读文档
- [[]] —

## 相关
- [[Deep Neural Networks for YouTube Recommendations]] — 该论文发表于此会议
- [[Paul Covington]] — 在此会议上发表论文
- [[YouTube]] — 通过此会议发表推荐系统研究成果