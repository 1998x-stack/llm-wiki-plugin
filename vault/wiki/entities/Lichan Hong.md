---
type: entity
entity_type: person
status: active
confidence: 0.8
created: 2026-04-26
updated: 2026-04-26
last_accessed: 2026-04-26
source_count: 2
tags: [推荐系统, 机器学习, 研究员]
aliases: [Lichan Hong]
relates_to:
  - target: "[[MMoE 论文]]"
    type: authored
  - target: "[[MMoE]]"
    type: contributed_to
  - target: "[[Google]]"
    type: affiliated_with
supersedes: null
---

# Lichan Hong

## 概述
Lichan Hong 是一位机器学习研究员，在 Google 工作期间参与了 MMoE (Multi-gate Mixture-of-Experts) 架构的研发工作，是该领域奠基性论文的作者之一。

## 关键内容

### 机构关联
- Google：在 Google 任职期间参与 MMoE 架构的研究和开发
- 作为论文第五作者参与了 MMoE 相关的 KDD 2018 论文

### 主要贡献
- **MMoE 论文合作者**：作为 KDD 2018 论文的重要作者之一，该论文提出了 MMoE 架构
- **多任务学习研究**：在多任务学习和推荐系统领域做出了贡献
- **算法研究**：参与了多门控混合专家网络算法的设计与研究

### 代表性工作
- MMoE (Multi-gate Mixture-of-Experts) 架构：KDD 2018 论文的核心贡献之一
- 该论文至今被引用数千次，是推荐系统多任务学习领域的奠基性工作之一

## 来源
- [[MMoE 论文]] — 作为论文作者
- raw/books/推荐系统/14-mmoe.md — 深度解读文章

## 相关
- [[MMoE 论文]] — 作为作者
- [[MMoE]] — 作为贡献者
- [[Google]] — 任职机构
- [[Jiaqi Ma]] — 论文合作者
- [[Ed H. Chi]] — 论文合作者