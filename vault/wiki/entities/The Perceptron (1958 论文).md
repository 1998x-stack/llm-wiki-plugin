...
## 概述
Rosenblatt 于 1958 年在 *Psychological Review* 发表的奠基性论文，首次提出[[感知机]]作为信息存储和组织在大脑中的概率模型，开创了可学习机器的先河。
...
2. **核心贡献**：提出[[感知机]]数学模型 ŷ = sign(w · x + b)，将单个神经元做最简洁的数学抽象，在特征空间中寻找超平面将两类样本分开。

3. **[[感知机学习规则]]**：首次提出权重通过错误自动修正的[[算法]]：w ← w + η · yᵢ · xᵢ，这是机器学习史上第一个自动学习[[算法]]。

4. **收敛定理**：严格证明若训练数据线性可分，[[感知机]]在有限步内必然收敛，更新次数上界为 T ≤ (R/γ)²。
