...
| **格式无关** | 支持文字型 PDF 和扫描件（[[光学字符识别|OCR]]）两种管道 |
...
1. **PDF 类型判断 & 后端选择**（TextBased / [[光学字符识别|OCR]]Based / MixedBased）
2. **底层 PDF 解析**（[[PyMuPDF]] 提取原始 Span/Block + 页面渲染为图像）
3. **[[文档布局检测]]**（[[DocLayout-YOLO]] / [[LayoutLMv3]] → 检测文本区/图/表/公式框）
4. **内容专项识别**（[[光学字符识别|OCR]]/[[PaddleOCR]]、[[公式识别]]/[[UniMERNet]]、[[表格识别]]/[[TableMaster]]）
...
- **TextBased**：适用于数字原生 PDF，[[PyMuPDF]] 直接读取亚像素级字符坐标，不需要 [[光学字符识别|OCR]]
- **[[光学字符识别|OCR]]Based**：适用于扫描件，页面渲染为图像后 [[PaddleOCR]] 全量识别
- **Mixed**：逐页判断，分别走对应管道
