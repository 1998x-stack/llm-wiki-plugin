...
## 概述
Box2D 是一个广泛使用的 2D 物理引擎，支持刚体模拟、碰撞检测、关节约束等功能。[[UrhoX引擎|UrhoX]] 的 2D 物理游戏脚手架集成了 Box2D。

## 关键内容
1. **在 UrhoX 中的集成**：2D 物理游戏脚手架（`scaffold-2d-physics.lua`）内置 Box2D 物理系统，用于平台跳跃等需要物理模拟的游戏类型。
