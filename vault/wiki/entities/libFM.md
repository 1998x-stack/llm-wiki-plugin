...
2. **优化方法**：提供三种优化算法——SGD（随机梯度下降，适合大规模在线学习）、ALS（交替最小二乘，回归任务稳定）、MCMC（[[马尔可夫链]]蒙特卡洛，[[托马斯·贝叶斯|贝叶斯]]推断可自动调节正则化强度）。
3. **线性复杂度**：利用 FM 的代数变换 $\frac{1}{2} \sum_{f=1}^{k} \left( (\sum_i v_{i,f} x_i)^2 - \sum_i v_{i,f}^2 x_i^2 \right)$，实现 $O(kn)$ 时间复杂度的前向计算和梯度更新，在稀疏数据下实际复杂度为 $O(k\bar{n})$。
