...
1. **核心思想**：[[LightGCN]] 虽然简化了 GCN，但仍需逐层进行[[邻域聚合]]的显式传播。UltraGCN 更进一步，直接逼近无限层图传播的极限，省掉了显式的多层传播过程。

2. **与 [[LightGCN]] 的关系**：UltraGCN 是 [[LightGCN]] "Less is More" 哲学的延续。如果 [[LightGCN]] 去掉了[[特征变换]]和[[非线性激活]]，UltraGCN 则去掉了显式传播本身。

3. **技术路径**：通过数学推导将无限层传播收敛到一个封闭解，避免了逐层[[矩阵]]乘法带来的[[计算]]开销。在保持甚至提升效果的同时，大幅降低了训练时间。
