...
## 概述
Flask 是一个轻量级 Python Web 框架（微框架），以简洁灵活著称，是 SWE-bench 评测中使用的真实开源仓库之一。

## 关键内容

1. **SWE-bench 中的角色**：Flask 是 SWE-bench 评测框架中使用的真实开源仓库之一，代表轻量级 Python Web 项目的代码库特征。

2. **与 Django 的对比**：相比 Django 的"全功能"设计，Flask 作为微框架代码库更精简，但同样需要 AI Agent 具备跨文件理解和多步骤修改能力。

3. **真实 Bug 场景**：SWE-bench 使用 Flask 仓库中真实用户报告的 Bug 作为评测任务，测试套件自动验证修复是否正确。
