---
type: entity
entity_type: paper
status: active
confidence: 0.98
created: 2026-04-17
updated: 2026-04-17
last_accessed: 2026-04-17
source_count: 1
tags:
- 技术
- 研究
- 历史
- 计算理论
aliases:
- No Silver Bullet
- Brooks 1986 论文
- 没有银弹
relates_to:
- target: "[[Frederick Brooks]]"
  type: caused_by
  confidence: 0.99
  note: 作者
- target: "[[软件危机]]"
  type: related_to
  confidence: 0.9
  note: 论文回应了软件危机的持续蔓延
- target: "[[供应链安全]]"
  type: compares_to
  confidence: 0.6
  note: 两者都涉及软件工程的根本困难
supersedes: null
---

# No Silver Bullet 论文

## 概述

[[Frederick Brooks]] 于1986年发表的《No Silver Bullet》，论证了软件工程中不存在能让生产力在十年内提高10倍的单一技术，因为核心困难来自本质复杂性而非偶然复杂性。

## 关键内容

### 论文信息

| 项目 | 内容 |
|------|------|
| **标题** | No Silver Bullet -- Essence and Accident in Software Engineering |
| **作者** | Frederick P. Brooks, Jr. |
| **发表时间** | 1986年（会议）；1987年4月（期刊） |
| **刊物** | IEEE Computer, Vol. 20, No. 4, pp. 10-19 |
| **被引次数** | 超过5000次 |

### 核心框架

**本质困难 vs 偶然困难**：
- **本质困难**：软件概念结构固有的复杂性，不可消除
- **偶然困难**：当前工具和表达方式的不完善，可通过技术进步消除

### 软件的四个本质特性

1. **复杂性（Complexity）**：软件是人类创造的最复杂的实体
2. **一致性（Conformity）**：必须适应外部世界的无序约束
3. **可变性（Changeability）**：因为可以改变所以总会被要求改变
4. **不可见性（Invisibility）**：无法像建筑那样被直观呈现

### 历史影响

- "没有银弹"成为软件工程中最著名的格言
- 建立了评价新技术的思维框架
- 影响了敏捷运动的思想根基

## 来源

- [[raw/books/计算机科学/16-brooks-no-silver-bullet.md]]

## 相关

- [[Frederick Brooks]] — 作者
- [[软件危机]] — 论文回应的背景
- [[供应链安全]] — 软件工程的另一维度
