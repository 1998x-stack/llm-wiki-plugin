...
1. **核心改进**：[[LightGCN]] 使用均匀的[[层组合]]权重 `α_k = 1/(K+1)`，[[LightGCN]]++ 引入自适应的嵌入范数缩放（embedding norm scaling），让不同层的嵌入根据其对最终预测的贡献自动调整权重。

2. **邻居加权**：在[[邻域聚合]]中，不再使用简单的[[对称归一化]] `1/√(|N_u|·|N_i|)`，而是引入可学习的邻居权重，使模型能[[区分]]不同邻居节点的重要性。
