...
3. **[[联合训练]]机制**：数学表达为 $P(Y=1|\mathbf{x}) = \sigma(\mathbf{w}_{wide}^T [\mathbf{x}, \phi(\mathbf{x})] + \mathbf{w}_{deep}^T a^{(l_f)} + b)$，Wide 用 FT[[强化学习|RL]]+L1 优化器（稀疏解），Deep 用 AdaGrad（自适应学习率），梯度从输出层同时反向传播到两组件。
