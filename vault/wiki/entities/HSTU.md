...
3. **HSTU 架构改造**（与标准 [[Transformer架构|Transformer]] 的关键区别）：
   - **SiLU [[注意力机制|注意力]]替代 [[Softmax]]**：[[注意力机制|注意力]]分数[[计算]]使用 $\phi_2(QK^\top + \text{relative\_attention\_bias}) \cdot V$，其中 $\phi_2$ 为 SiLU 激活而非 [[Softmax]]。保留偏好强度信息，适应非平稳分布。[[Ablation Study|消融实验]]证实 [[Softmax]] 替换导致性能下降。
   - **相对[[注意力机制|注意力]]偏置替代[[绝对位置编码]]**：将位置信息和时间间隔作为相对偏置直接注入[[注意力机制|注意力]]分数，同时编码序列顺序和交互时间距离。
   ...
   - **精简架构**：[[注意力机制|注意力]]外线性层从 6 个减少到 2 个，激进融合[[计算]]操作，降低激活内存。
   - **[[因果掩码]]**：单向[[注意力机制|注意力]]，确保[[AR 模型（自回归模型）|自回归]]生成合法性。
