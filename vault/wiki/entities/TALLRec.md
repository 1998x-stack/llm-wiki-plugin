...
## 概述
首个尝试用大[[Language-Model|语言模型]] [[LLaMA]] 进行推荐任务微调的工作，使用[[指令调优]]使 [[LLaMA]] 适配推荐场景。

## 关键内容

1. **开创性贡献**：首个尝试用大[[Language-Model|语言模型]]（[[LLaMA]]）进行推荐任务微调的工作，标志着 LLM 推荐从 T5 时代（60M-223M 参数）进入 [[LLaMA]] 时代（7B+ 参数）。
