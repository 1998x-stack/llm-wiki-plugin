...
## 概述
Minsky 和 Papert 于 1969 年出版的著作，严格证明了单层感知机无法解决线性不可分问题（如 XOR），直接导致神经网络研究进入长达十余年的 [[AI 寒冬]]。

## 关键内容

1. **核心定理**：证明了单层感知机无法解决线性不可分问题，以 XOR（异或）为经典反例——无法用一条直线将 (0,0)/(1,1) 与 (0,1)/(1,0) 分开。
...
4. **后续修正**：Minsky-Papert 的结论仅适用于单层感知机。[[多层感知机（MLP）]]配合非线性激活和[[反向传播（Backpropagation）|反向传播算法]]最终突破了这一限制。
