...
- **在 [[Hermes Agent|Hermes]] 中的角色**：[[闭环学习系统]]的关键组成部分，与[[Memory-Management|记忆管理]]、[[Skills|Skill]] 创建、FTS5 召回协同工作；通过 `honcho-ai` 包集成，可用 `hermes honcho` 查看状态、`hermes honcho reset` 重置
- **工作方式**：在 Agent 跨会话交互中持续收集用户偏好、环境信息、[[项目约定手册|项目约定]]等，形成结构化的用户模型
