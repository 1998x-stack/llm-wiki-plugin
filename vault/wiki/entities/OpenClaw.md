...
Pi 原本是 [[Mario-Zechner]] 的私人工具，"永远不会有用户"。OpenClaw 作为 Pi 的第一个大规模用户验证了其架构的可扩展性——从个人 CLI 工具到服务百万用户的多渠道平台，核心引擎无需重写。

### 3. 与 agentskills.io 开放标准的关系

OpenClaw 的技能格式兼容 [[agentskills.io]] 开放规范，支持通过 `hermes claw migrate` 工具从 OpenClaw 迁移到 [[Hermes Agent]]。迁移命令支持 `--dry-run` 预览、`--preset user-data` 只迁移用户数据、`--overwrite` 覆盖冲突文件。这体现了[[开放技能标准]]的核心价值：技能可在不同 Agent 框架间流通，而非某个系统的私有数据。
