...
2. **共享嵌入**：FM 和 DNN 共享类别型特征的[[嵌入表示]]，避免了分别学习导致的参数冗余和不一致性。这种"浅层 + 深层"的并行设计模式已成为推荐系统的标准架构。共享嵌入带来三重优势：参数效率、联合优化、消除预训练。
3. **无需手工特征**：DeepFM 的端到端设计消除了对人工构造交叉特征的依赖，模型自动学习[[特征交叉|特征交互]]，大幅降低了特征工程成本。相比 [[Wide & Deep]] 的 Wide 部分需要手工 cross-product 特征，DeepFM 用 FM 完全替代。
4. **与 FM 的关系**：DeepFM 直接在架构中保留了完整的 [[Factorization Machines|FM]] 组件作为"浅层"模块，是 [[Factorization Machines|FM]] 思想在深度学习时代的自然延伸。[[Factorization Machines|FM]] 负责低阶交互的显式建模，DNN 补足高阶交互的隐式学习，两者互补。
...
6. **后续影响**：DeepFM 的"浅层 + 深层"并行架构启发了后续一系列工作，包括 DCN（Google 2017，Cross Network 显式交叉）、xDeepFM（微软 2018，CIN 向量级显式交叉）、AutoInt（2019，多头自注意力可解释交互）等。
