...
1. **核心问题**：LLM 推荐模型（如 P5 [[规范化理论|范式]]）需要大量训练数据，但并非所有数据对模型学习都有同等贡献。DEALRec 通过数据剪枝识别和保留最有价值的训练样本。

2. **技术方法**：通过数据剪枝（data pruning）方法筛选训练数据，去除冗余或低信息量的样本，提升 LLM 推荐的训练效率。

3. **在 LLM 推荐谱系中的位置**：属于[[生成式 LLM 推荐]][[规范化理论|范式]]，关注训练效率优化，与 [[TALLRec]]、[[InstructRec]]、[[LC-Rec]] 等工作共同推动了 LLM 推荐的实用化。
