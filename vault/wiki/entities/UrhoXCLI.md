...
4. **[[FBX格式|FBX]] 模型导入**：`import-model` 子命令将 [[FBX格式|FBX]] 转为引擎 [[最小描述长度原理|MDL]] 格式；可选生成材质 XML、纹理（含 sidecar `.xml` 配置）、预制体和 ANI 动画；各产物路径独立，不传则跳过。默认开启 3 级 [[LOD（细节层次）]] 自动生成（85%/65%/45% 简化比例），可用 `--no-lod` 关闭或 `--lod-levels` 调整级数。
5. **GLB/[[GLB_glTF格式|glTF]] 模型导入**：`import-gltf` 子命令将 [[GLB_glTF格式|GLB/glTF]] 转为引擎 [[最小描述长度原理|MDL]] 格式；自动处理右手系→左手系坐标转换、UV 翻转和单位转换；GLB 内嵌纹理自动提取；输出产物（[[最小描述长度原理|MDL]]、材质、纹理、预制体、动画）均独立可选，不传则跳过。
6. **[[最小描述长度原理|MDL]] 模型信息查询**：`model-info -i <mdl>` 输出包围盒（Min/Max/Size）、Geometry 数量及每个几何体的 [[LOD（细节层次）|LOD]] 层级（顶点数/三角面数/切换距离）、顶点属性、骨骼数；加 `--bones` 列出完整骨骼列表（骨骼名+父索引），用于导入验证和 [[动画重定向]] 前检查。
