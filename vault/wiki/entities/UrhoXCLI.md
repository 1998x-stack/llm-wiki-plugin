...
4. **FBX 模型导入**：`import-model` 子命令将 FBX 转为引擎 MDL 格式；可选生成材质 XML、纹理（含 sidecar `.xml` [[Configuration|配置]]）、预制体和 ANI 动画；各产物路径独立，不传则跳过。默认开启 3 级 [[LOD（细节层次）]] 自动生成（85%/65%/45% 简化比例），可用 `--no-lod` 关闭或 `--lod-levels` 调整级数。
