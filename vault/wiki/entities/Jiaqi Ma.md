---
type: entity
entity_type: person
status: active
confidence: 0.85
created: 2026-04-16
updated: 2026-04-16
last_accessed: 2026-04-16
source_count: 1
tags: [推荐系统, 多任务学习, 研究者]
aliases: []
relates_to: [MMoE 论文, MMoE, Google]
supersedes: null
---

# Jiaqi Ma

## 概述
推荐系统研究者，[[MMoE 论文]]第一作者，当时为密歇根大学博士生，在 [[Google]] 实习期间完成该奠基性工作。

## 关键内容

### 学术背景
- 密歇根大学博士生期间在 [[Google]] 实习
- 在 [[Google]] 推荐系统团队实习期间完成 MMoE 研究工作
- 合作者包括 [[Zhe Zhao]], Xinyang Yi, Jilin Chen, Lichan Hong, [[Ed H. Chi]]

### 主要贡献
- **[[MMoE 论文]]**(KDD 2018)：提出 [[MMoE|Multi-gate Mixture-of-Experts]] 架构
  - 为每个任务配备独立门控网络，解决[[多任务学习]]中的[[负迁移]]问题
  - 在 [[YouTube]] 大规模推荐系统中验证有效性
  - 至今被引用数千次，成为推荐系统[[多任务学习]]的事实标准
- 建立通过合成数据精确控制任务相关性来验证模型的实验[[规范化理论|范式]]

### 影响
- MMoE 开创了"Expert + Gate"[[规范化理论|范式]]，催生大量后续研究(PLE、ESMM 等)
- 该工作推动了[[多任务学习]]在工业界推荐系统的广泛采纳

## 来源
- raw/books/推荐系统/14-mmoe.md — MMoE 深度解读文章

## 相关
- [[MMoE 论文]] — 第一作者的奠基性工作
- [[Ed H. Chi]] — 通讯作者，Google Research 杰出科学家
- [[Google]] — 实习机构
