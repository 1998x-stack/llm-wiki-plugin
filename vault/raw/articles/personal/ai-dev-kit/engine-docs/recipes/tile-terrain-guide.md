...
# 3. 初始化并绘制纹理
subprocess.run([CLI, 'init-weightmap', '--terrain', '/workspace/.tmp/terrain.json',
    '--output', '/workspace/.tmp/weights.bin'], check=True)
subprocess.run([CLI, 'paint-texture', '--weightmap', '/workspace/.tmp/weights.bin',
    '--pos', '50,50', '--layer', '1', '--radius', '30'], check=True)

# 4. 生成 HLOD 场景（输出必须在 /workspace/assets/ 子目录下）
subprocess.run([CLI, 'generate', '--type', 'hlod',
    '--input', '/workspace/.tmp/terrain.json',
    '--pak', pak,
    '--tileset-dir', 'TileSets',
    '--weightmap', '/workspace/.tmp/weights.bin',
