...
UI.Init({
    ...
    -- 主题（可选，默认使用内置 Material 风格主题）
    -- theme = customTheme,

    -- 字体大小模式（可选，默认 "pixel"）
    -- fontSizeMethod = "pixel",   -- "pixel"（pt→px 转换）或 "char"（直接使用 pt，不转换）

    -- 自动事件订阅（可选，默认全部启用）
    -- 通常不需要设置此项，省略等同于 true。
    -- 仅在需要手动调用 UI.Update()/UI.Render() 时才设为 false。
    -- autoEvents = true,       -- （默认）启用全部：input + update + render
    -- autoEvents = false,      -- 禁用全部，需手动调用 UI.Update()/UI.Render()
    -- autoEvents = { input = true, update = true, render = false },  -- 细粒度控制

    -- 分辨率缩放（详见「10. 渲染 > 分辨率缩放」）
    scale = UI.Scale.DEFAULT,  -- 要求默认使用，遵循 Web/CSS 像素标准：1基准像素 = 1CSS像素，小屏 UI 密度自适应
})
```
...
```lua
UI.Init({ scale = UI.Scale.DEFAULT })  -- 默认，即 UI.Scale.DPR_DENSITY_ADAPTIVE
```
