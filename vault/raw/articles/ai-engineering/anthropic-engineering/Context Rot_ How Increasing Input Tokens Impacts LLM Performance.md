...
![Context Rot: How Increasing Input Tokens Impacts LLM Performance](https://www.trychroma.com/img/context_rot/header_plot.jpg)![Context Rot: How Increasing Input Tokens Impacts LLM Performance](https://www.trychroma.com/img/context_rot/hero_plot.png)

Claude Sonnet 4, GPT-4.1, Qwen3-32B, and Gemini 2.5 Flash on Repeated Words Task

Recent developments in LLMs show a trend toward longer context windows, with the input token count of the latest models reaching the millions. Because these models achieve near-perfect scores on widely adopted benchmarks like Needle in a Haystack (NIAH) \[[1](#niah)\], it’s often assumed that their performance is uniform across long-context tasks.
...
Additionally, we include a conversational question-answer evaluation using LongMemEval \[[2](#longmemeval)\], as well as a synthetic task in which models replicate a series of repeated words. Each task remains intentionally simple and is deliberately controlled to isolate the impact of context length alone.
...
It is common for modern LLMs to have input context lengths in the millions of tokens. Gemini 1.5 Pro \[[3](#gemini-1.5-pro)\] first introduced their 1M context window in early 2024, followed by the recent GPT-4.1’s 1M context window \[[4](#gpt-4.1)\] and Llama 4 with 10M \[[5](#llama-4)\]. The use case for long context is compelling: longer context means that the LLM can process more information with each call and generate more informed outputs.
...
Models often have to deal with distractors as well, which has been shown to degrade performance \[[11](#distractors)\].
...
We evaluate each model across its maximum context window with temperature=0 unless that setting is incompatible (i.e. o3) or explicitly discouraged (i.e. Qwen’s “thinking mode”). For Qwen models, we apply the YaRN method \[[13](#yarn)\] to extend from 32,768 to 131,072 tokens.
...
3. Use UMAP \[[14](#umap)\] for dimensionality reduction with the following parameters: n\_neighbors=30, min\_dist=0.05, n\_components=50, random\_state=42
4. Use HDBSCAN \[[15](#hdbscan)\] to create clusters with the following parameters: min\_cluster\_size=10, min\_samples=15
