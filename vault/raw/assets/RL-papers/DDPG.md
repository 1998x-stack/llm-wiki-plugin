...
The return from a state is deﬁned as the sum of discounted future reward Rt = (cid:80)T
i=t γ(i−t)r(si, ai)
...
(cid:2)r(st, at) + γ Eat+1∼π [Qπ(st+1, at+1)](cid:3)
...
L(θQ) = Est∼ρβ ,at∼β,rt∼E

(cid:104)(cid:0)Q(st, at|θQ) − yt

(cid:1)2(cid:105)

where

yt = r(st, at) + γQ(st+1, µ(st+1)|θQ).
...
(cid:2)∇θµQ(s, a|θQ)|s=st,a=µ(st|θµ)
(cid:2)∇aQ(s, a|θQ)|s=st,a=µ(st)∇θµµ(s|θµ)|s=st
...
directly copying the weights. We create a copy of the actor and critic networks, Q(cid:48)(s, a|θQ(cid:48)
) and
µ(cid:48)(s|θµ(cid:48)
) respectively, that are used for calculating the target values. The weights of these target
networks are then updated by having them slowly track the learned networks: θ(cid:48) ← τ θ + (1 −
τ )θ(cid:48) with τ (cid:28) 1. This means that the target values are constrained to change slowly, greatly
...
robust solutions exist. We found that having both a target µ(cid:48) and Q(cid:48) was required to have stable
...
from the learning algorithm. We constructed an exploration policy µ(cid:48) by adding noise sampled from
...
Initialize target network Q(cid:48) and µ(cid:48) with weights θQ(cid:48)
...
Set yi = ri + γQ(cid:48)(si+1, µ(cid:48)(si+1|θµ(cid:48)
...
CartPendulum Swing-upCartpole Swing-upFixed ReacherBlockworldGripperPuck ShootingMonoped BalancingMoving GripperCheetahMillion Steps01101100110010011100Normalized Reward1000001111Published as a conference paper at ICLR 2016
...
Rbest,pix Rav,cntrl Rbest,cntrl
-0.080
-0.139
0.125
-0.045
...
ously supposed. Wawrzy´nski (2009); Wawrzy´nski & Tanwani (2013) has trained stochastic policies

7

PendulumCheetahCartpoleEstimated QReturnPublished as a conference paper at ICLR 2016
...
computational demand, PILCO is “impractical for high-dimensional problems” (Wahlstr¨om et al.,
...
Wahlstr¨om et al. (2015) used a deep dynamical model network along with model predictive control
...
Gl¨ascher, Jan, Daw, Nathaniel, Dayan, Peter, and O’Doherty, John P. States versus rewards: dis-
...
Koutn´ık, Jan, Schmidhuber, J¨urgen, and Gomez, Faustino. Evolving deep unsupervised convolu-
...
Koutn´ık, Jan, Schmidhuber, J¨urgen, and Gomez, Faustino. Online evolution of deep convolutional
...
Wahlstr¨om, Niklas, Sch¨on, Thomas B, and Deisenroth, Marc Peter. From pixels to torques: Policy
...
Wawrzy´nski, Paweł. Real-time reinforcement learning by sequential actor–critics and experience

replay. Neural Networks, 22(10):1484–1497, 2009.

Wawrzy´nski, Paweł. Control policy with autocorrelated noise in reinforcement learning for robotics.

International Journal of Machine Learning and Computing, 5:91–95, 2015.

Wawrzy´nski, Paweł and Tanwani, Ajay Kumar. Autonomous reinforcement learning with experience
...
[− 1√
f ] where f is the fan-in of the layer. The actions were not included until the fully-connected
...
gripper to grab a block and lift it against gravity to a ﬁxed target posi-
tion.
...
very closely on the one introduced by Wawrzy´nski (2009); Wawrzy´nski
& Tanwani (2013).
...
Agent must use an arm with gripper attached to a moveable platform to
...
Agent is required to move a 7-DOF human-like arm to a ﬁxed target
position.
