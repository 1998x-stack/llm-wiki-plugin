---
description: "一键维护：relink → check → lint → build 全流水线"
---

# wiki:maintain

一键执行完整知识库维护流水线：relink → check → lint → build。

> 等价于依次运行 `wiki:relink`、`wiki:check`、`wiki:lint`、`wiki:build`，但在关键步骤失败时提前终止。
> 需要重建索引和主题分类？请单独运行 `wiki:reindex`。
...
基于步骤 2 的诊断结果，执行 `wiki:lint` 的修复流程：
...
```bash
git add -A && git commit -m "chore: wiki:maintain — relink + check + lint + build (YYYY-MM-DD)"
```
