...
- `推荐系统` — 协同过滤、矩阵分解、深度学习推荐、CTR 预估（216 页）
- `AI工程` — LLM 上下文工程、RAG、Agent 基础设施、Claude Code（87 页）
- `数值分析` — 数值方法、ODE 求解、插值与积分（62 页）
- `机器人学` — 运动规划、机器人控制、感知定位（56 页）
- `信息论` — Shannon 信息论、编码理论、压缩理论（51 页）
- `概率论` — 概率公理、随机过程、统计推断、时间序列（46 页）
- `工具与框架` — 开发工具、CLI、LSP、语言服务器、数据 API（37 页）
- `矩阵理论` — 线性代数、特征值理论、数值线性代数（32 页）
- `强化学习` — DQN 系列、策略梯度、Actor-Critic、最大熵 RL（30 页）
- `Agent系统` — AI Agent 架构、工作流、多 Agent 协调（31 页）
- `经济学` — 宏观经济学、凯恩斯主义、增长理论（26 页）
- `Lua编程` — Lua 语言特性、元表、模块、OOP、性能优化（25 页）
- `LLM能力` — 上下文压缩、扩展思维、提示词缓存（23 页）
- `脑科学` — 神经科学历史、突触可塑性、感知神经元（23 页）
- `游戏开发` — 游戏引擎架构、ECS、Love2D、Defold、Roblox（22 页）
- `文档处理` — PDF 解析、OCR、文档结构提取（20 页）
- `社会科学` — 社会学、规则漏洞学、婚姻研究（17 页）
- `时间序列` — ARIMA、GARCH、Transformer 预测模型（15 页）
- `控制论` — 控制论、系统理论、自创生、耗散结构（15 页）
- `C++编程` — C++ 语言特性、STL、OOP、LSP 工具（10 页）
- `AI设计` — AI 辅助 UI/UX 设计、设计规则系统（8 页）
- `计算理论` — 图灵机、停机问题、算法信息论（5 页）
- `其他` — 小型 cluster 合并（3 页）
